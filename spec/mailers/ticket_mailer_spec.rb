...
RSpec.describe TicketMailer, type: :mailer do
  describe "ticket_email" do
    ...
    let!(:order) { create(:order, user_id: user.id, event_id: event.id, purchased_ticket_ids: [purchased_ticket.id])}
    let(:mail) { TicketMailer.with(ticket_details: [purchased_ticket], order_amount: ticket.price, event: event, order: order).ticket_email }
